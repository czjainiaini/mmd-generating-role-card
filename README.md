...
- 标签化角色人设、开场白与独立世界书
- HUD、侧栏、消息气泡、输入区、官方主题桥与受控宿主弹窗换肤
...
- 复杂舞台可选 Vue 3；Motion、PixiJS、Three.js、Howler、DOMPurify 与构建期 Ajv 按玩法和风险选择。外链固定版本并在目标站验证白名单，失败时保留核心文字玩法。
...
它会检查 Skill 结构、相对链接、SDK 契约生成结果、手册代码夹具、正则 / 世界书保真工具回归、示例正则和示例世界书，并扫描常见凭据与本机绝对路径。

## 平台边界

- 本仓库不是酒馆角色卡工具，不会把 Tavern Helper、MVU 或 STScript 当成 MMD API。
- MMD 的父页面弹窗不能被角色脚本跨源任意修改；公开换肤仅使用官方主题桥的支持颜色变量，或从精确 `[data-host="…"]` 根开始、经过平台过滤的宿主 CSS。后者不是 SDK，不允许读写父页面 DOM、改平台业务或依赖裸内部 class。

...
平台自己的 class 名会变，**不要抄页面上看到的** `.xxx` **去当 iframe 选择器**。聊天页里能稳定写进样式和脚本的是下面这些 `data-chat` / `data-slot`。

你自己画的 HTML，用你自己起的 class（见[第 1 章](01-style.md)）。不要在自己的标签上写 `data-*`，净化会删掉。

宿主弹窗是唯一单列的例外：官方 ZIP 记录平台会过滤并注入以 `[data-host="…"]` 开头的 CSS。它只在 `<style>` 中作受控定位，不能让脚本访问父页面，也不能把父页面 class 单独拿来写选择器。开放根、当前结构记录、过滤边界和验收方式见[宿主弹窗探针](../references/host-popup.md)。
...
`[data-host]` 出现在父页面的开放弹窗根上，不是聊天 iframe 的稳定 DOM，也不是作者可自行添加的属性。写宿主样式时只使用下面的受控形状：

```css
[data-host="model-setting"] .mp-card { /* 仅在当前目标站取证后才可使用的内部后代 */ }
```

最左侧必须是一个开放 `[data-host="…"]` 根；后面才可以跟已取证的 class。禁止裸 `.mp-card`、`.model-setting-scope` 或全局 `.btn`；不要在宿主选择器里使用 `:has()`、`url(` 或 CSS 原生嵌套，它们会被过滤。`data-host` 也不能放进作者输出 HTML。

根名是通路白名单，不是永久内部 API：`models` 和 `model-setting` 分开，`persona-confirm` 也不在 `persona` 树中；`summary` 的主面板和编辑层在同一根内。当前已知范围以及“资料记录 / 真实目标站已验收”的区别见[宿主弹窗探针](../references/host-popup.md)。

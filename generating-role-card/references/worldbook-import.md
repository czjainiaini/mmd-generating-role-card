...
- 原始导入文件用于格式修复时，保留原文语义、UID、触发词、条目顺序与启用状态；默认只修字段形状、类型与明确的旧占位符。未知根 / 条目字段必须停止并报告，不能为了“通过”而静默删除。
- 默认不改正文标签；用户明确要求修复简单缺闭合标签时，才给 `normalize-worldbook.mjs` 加 `--repair-tags`。复杂交叉嵌套必须逐段修复，不能把内容改写伪装成格式修复。
...
要修复已确认的简单缺闭合标签时，才在同一命令末尾增加 `--repair-tags`。
...
打包默认遇到超限或无法完整合并的条目就报错，避免截断或悄悄丢设定。用户明确同意只合并一部分时，才追加 `--allow-partial`，并在交付中列出遗漏条目。

## 保真转换与兼容降级

- 标准化应可重复运行而不继续改动结果。已配置的主次关键词即使当前常驻也保留，便于之后切换模式；概率、插入位置、扫描窗口等平台行为不能靠压入人设等价复现。
- 已停用或零概率内容保留在独立世界书中，但不参与兼容打包。低于 100% 的内容若压入人设，只能带文字说明，不能伪装成每轮必然生效。
- 压入人设是目标入口不支持独立世界书时的兼容降级：必须明确说明它会损失平台触发 / 概率 / 插入机制，不能宣称完全等价。
- 本文件的相对脚本命令以 skill 目录为当前目录；实际执行时使用脚本和输入 / 输出文件的绝对路径。
